...
void drawCubeFromOBJ(const char *filename, struct objCube* Cube) {
    FILE *file = fopen(filename, "r");
    if (!file) {
        printf("Hiba: Nem sikerult megnyitni az obj fajlt!\n");
        return;
    }
...
int main(int argc, char** argv) {
    ...
    if (!context) {
        printf("OpenGL kontextus létrehozása sikertelen: %s\n", SDL_GetError());
    ...
    if (IMG_Init(IMG_INIT_JPG) == 0) {
        printf("SDL_image inicializálása sikertelen: %s\n", IMG_GetError());
